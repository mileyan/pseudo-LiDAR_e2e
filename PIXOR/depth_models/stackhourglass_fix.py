...
class PSMNet(nn.Module):
    ...
    def warp(self, x, calib):
        ...
        x = x.view(B, -1, C, D)
        # mesh grid
        xx = (calib / (self.down*4.))[:, None] / torch.arange(1, 1 +
                                                              self.maxdepth//self.down, device='cuda').float()[None, :]
        new_D = int(self.maxdepth//self.down)
        xx = xx.view(B, 1, new_D).repeat(1, C, 1)
        xx = xx.view(B, C, new_D, 1)
        yy = torch.arange(0, C, device='cuda').view(-1,
                                                    1).repeat(1, new_D).float()
    ...
    def forward(self, left, right, calib, out_std=False, out_cost_volume=False):
        ...
        if self.training:
            ...
            cost2 = F.upsample(cost2, [self.maxdepth, left.size()[
                               2], left.size()[3]], mode='trilinear')
        ...
        cost3 = F.upsample(cost3, [self.maxdepth, left.size()[
                           2], left.size()[3]], mode='trilinear')
