...
from lib.net.depth_net import DepthModel
import lib.net.train_functions as train_functions
from lib.datasets.kitti_depth_rcnn_dataset import KittiRCNNDataset
...
parser.add_argument("--train_mode", type=str, default='rpn',
...
parser.add_argument('--mgpus', action='store_true',
                    default=False, help='whether to use multiple gpu')
...
parser.add_argument('--train_with_eval', action='store_true',
                    default=False, help='whether to train with evaluation')
...
parser.add_argument("--max_high", type=float, default=1.0)
...
def create_dataloader(logger):
    ...
    if not args.gt_database:
        args.gt_database = None
...
def create_scheduler(optimizer, total_steps, last_epoch):
    ...
    if cfg.TRAIN.OPTIMIZER == 'adam_onecycle':
        lr_scheduler = lsf.OneCycle(
            optimizer, total_steps, cfg.TRAIN.LR, list(
                cfg.TRAIN.MOMS), cfg.TRAIN.DIV_FACTOR, cfg.TRAIN.PCT_START
        )
    else:
        lr_scheduler = lr_sched.LambdaLR(
            optimizer, lr_lbmd, last_epoch=last_epoch)

    bnm_scheduler = train_utils.BNMomentumScheduler(
        model, bnm_lmbd, last_epoch=last_epoch)
...
if __name__ == "__main__":
    ...
    if args.train_mode == 'rpn':
        ...
        root_result_dir = os.path.join('../', 'output', 'rcnn', cfg.TAG)
    elif args.train_mode == 'end2end':
        cfg.RCNN.ENABLED = True
        cfg.RPN.ENABLED = True
        cfg.RPN.FIXED = False
        root_result_dir = os.path.join('../', 'output', 'end2end', cfg.TAG)
    else:
        raise NotImplementedError
    ...
    # tensorboard log
    tb_log = SummaryWriter(os.path.join(root_result_dir, 'tensorboard'))
    ...
    # Create depth model
    depth_model = DepthModel(maxdepth=80, maxdisp=192, down=2,
                             pretrain=args.depth_ckpt,
                             lr=cfg.TRAIN.DEPTH_LR,
                             save_tag=cfg.TAG,
                             mgpus=args.mgpus,
                             mode='TRAIN')

    if args.fix_bn:
        for _, m in enumerate(depth_model.net.modules()):
    ...
    if args.ckpt is not None:
        ...
        if args.finetune:
            train_utils.load_checkpoint(pure_model, filename=args.ckpt, logger=logger)
        else:
            it, start_epoch = train_utils.load_checkpoint(
                pure_model, optimizer, filename=args.ckpt, logger=logger)
    ...
    if args.rpn_ckpt is not None:
        ...
        train_utils.load_part_ckpt(
            pure_model, filename=args.rpn_ckpt, logger=logger, total_keys=total_keys)
    ...
    if args.fix_bn:
        if cfg.RPN.ENABLED:
            for _, m in enumerate(model.module.rpn.modules() if args.mgpus
                                  else model.rpn.modules()):
                if 'BatchNorm' in m.__class__.__name__:
                    #pdb.set_trace()
                    try:
                        m.bn
