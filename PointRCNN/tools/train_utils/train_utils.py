...
class Trainer(object):
    def __init__(self, model, depth_model, model_fn, optimizer, ckpt_dir, lr_scheduler, bnm_scheduler,
                 model_fn_eval, tb_log, eval_frequency=1, lr_warmup_scheduler=None, warmup_epoch=-1,
                 grad_norm_clip=1.0, max_high=1.0):
        self.model, self.depth_model, self.model_fn, self.optimizer, self.lr_scheduler, self.bnm_scheduler, self.model_fn_eval = \
            model, depth_model, model_fn, optimizer, lr_scheduler, bnm_scheduler, model_fn_eval
    ...
    def _train_it(self, batch):
        ...
        depth_loss, new_batch = self.depth_model.train(batch)
        loss, tb_dict, disp_dict = self.model_fn(self.model, new_batch)
    ...
    def eval_epoch(self, d_loader):
        ...
        for i, data in tqdm.tqdm(enumerate(d_loader, 0), total=len(d_loader), leave=False, desc='val'):
            ...
            depth_loss, new_data = self.depth_model.eval(data)
            loss, tb_dict, disp_dict = self.model_fn_eval(self.model, new_data)
            ...
            for epoch in tbar:
                ...
                for cur_it, batch in enumerate(train_loader):
                    ...
                    disp_dict.update({'loss': loss, 'lr': cur_lr,
                                      'depth_lr': self.depth_model.optimizer.param_groups[0]['lr']})
                    ...
                    if self.tb_log is not None:
                        ...
                        self.tb_log.add_scalar('learning_rate', cur_lr, it)
                        self.tb_log.add_scalar('depth_loss', disp_dict['depth_loss'], it)
                        for key, val in tb_dict.items():
                            self.tb_log.add_scalar('train_' + key, val, it)
                            
                self.depth_model.scheduler.step()
                # save trained model
                ...
                if trained_epoch % ckpt_save_interval == 0:
                    ...
                    self.depth_model.save_checkpoint(epoch, filename='checkpoint_epoch_%d.pth.tar' % trained_epoch)
