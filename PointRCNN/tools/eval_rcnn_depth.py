...
parser.add_argument("--depth_ckpt", type=str, default=None, help="specify the checkpoint of rcnn if trained separated")
...
def eval_one_epoch_rpn(model, depth_model, dataloader, epoch_id, result_dir, logger):
    np.random.seed(1024)
    ...
    for data in dataloader:
        detph_loss, data = depth_model.eval(data)
...
def eval_one_epoch_rcnn(model, depth_model, dataloader, epoch_id, result_dir, logger):
    np.random.seed(1024)
...
def eval_one_epoch_joint(model, depth_model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        disp_dict = {'mode': mode, 'recall': '%d/%d (%.4f)' % (total_recalled_bbox_list[3], total_gt_bbox,
                                                               total_recalled_bbox_list[3]/total_gt_bbox)}
        progress_bar.set_postfix(disp_dict)
        progress_bar.update()
...
def eval_one_epoch(model, depth_model, dataloader, epoch_id, result_dir, logger):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ret_dict = eval_one_epoch_rpn(model, depth_model, dataloader, epoch_id, result_dir, logger)
    elif not cfg.RPN.ENABLED and cfg.RCNN.ENABLED:
        ret_dict = eval_one_epoch_rcnn(model, depth_model, dataloader, epoch_id, result_dir, logger)
...
def eval_single_ckpt(root_result_dir):
    ...
    root_result_dir = os.path.join(root_result_dir,
...
def repeat_eval_ckpt(root_result_dir, ckpt_dir):
    ...
    # Create depth model
    depth_model = DepthModel(maxdepth=80, maxdisp=192, down=2,
                             pretrain='../depth_network/results/stack_finetune_from_sceneflow/checkpoint.pth.tar',
                             save_tag=cfg.TAG,
                             mode='TEST',
                             mgpus=args.mgpus)
    ...
    while True:
        ...
        tb_dict = eval_one_epoch(model, depth_model, test_loader, cur_epoch_id, cur_result_dir, logger)
