...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='Car', mode='TRAIN', random_select=True,
                 ...
                 rcnn_eval_feature_dir=None, gt_database_dir=None,  pseudo_lidar=False):
    ...
    def get_image(self, idx, left_image=True):
        return super().get_image(idx % 10000, left_image)
    ...
    # @staticmethod
    # def depth_from_lidar(pc_velo, H, W, calib):
    #     pts_2d, _ = calib.lidar_to_img(pc_velo)
    #     fov_inds = (pts_2d[:, 0] < W - 1) & (pts_2d[:, 0] >= 0) & \
    #         (pts_2d[:, 1] < H - 1) & (pts_2d[:, 1] >= 0)
    #     fov_inds = fov_inds & (pc_velo[:, 0] > 2)
    #     imgfov_pc_velo = pc_velo[fov_inds, :]
    #     imgfov_pts_2d = pts_2d[fov_inds, :]

    #     imgfov_pc_rect = calib.lidar_to_rect(imgfov_pc_velo)
    #     depth_map = np.zeros((H, W)) - 1
    #     imgfov_pts_2d = np.round(imgfov_pts_2d).astype(int)

    #     depth_map[imgfov_pts_2d[:, 1], imgfov_pts_2d[:, 0]] = imgfov_pc_rect[:, 2]
