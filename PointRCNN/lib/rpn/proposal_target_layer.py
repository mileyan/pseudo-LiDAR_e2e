...
import lib.utils.simple_roipool3d.simple_roipool3d_utils as simple_roipool3d_utils
...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        batch_size, num_box, pts_num = pts_idx.size()[0], pts_idx.size()[1], pts_idx.size()[2]
        fea_ch = pts_feature.size()[2]

        pts_idx = pts_idx.view(batch_size, -1, 1) # [batch_size, num_box * 512, 1]
        pts_idx = pts_idx.repeat(1, 1, fea_ch + 3) # [batch_size, num_box * 512, ch + 3]

        # [batch_size, num_box * 512, ch + 3]
        pooled_features = torch.gather(torch.cat([rpn_xyz, pts_feature], dim=2), 1, pts_idx.long())
        pooled_features = pooled_features.view(batch_size, num_box, pts_num, -1)
