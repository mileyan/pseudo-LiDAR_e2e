import torch
import simple_roipool3d_cuda
import numpy as np
import lib.utils.kitti_utils as kitti_utils


def simple_roipool3d_gpu(pts, pts_feature, boxes3d, pool_extra_width, sampled_pt_num=512):
    """
    :param pts: (B, N, 3)
    ...
    pts_idx = torch.cuda.IntTensor(torch.Size((batch_size, boxes_num, sampled_pt_num))).zero_()
    pooled_empty_flag = torch.cuda.IntTensor(torch.Size((batch_size, boxes_num))).zero_()

    simple_roipool3d_cuda.forward(pts.contiguous(), pooled_boxes3d.contiguous(),
                           pts_feature.contiguous(), pts_idx, pooled_empty_flag)
